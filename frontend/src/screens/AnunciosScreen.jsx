...
export default function AnunciosScreen({
  ...
  const [announcementDateFilter, setAnnouncementDateFilter] = useState("todos");
  const [announcementDateDropdownOpen, setAnnouncementDateDropdownOpen] = useState(false);
  ...
  useEffect(() => {
    setPage(1);
  }, [condoParam, announcementDateFilter]);

  useEffect(() => {
    setLoading(true);
    const dateFilter = announcementDateFilter !== "todos" ? announcementDateFilter : undefined;
    api.getAnunciosPaged({ page, limit: PAGE_SIZE, condo: condoParam, dateFilter })
      .then(setPageData)
      .catch((err) => onToast?.(err.message, "error"))
      .finally(() => setLoading(false));
  }, [page, condoParam, announcementDateFilter, anunciosData]);

  const [deletingAnuncioId, setDeletingAnuncioId] = useState(null);

  const confirmDeleteAnuncio = async () => {
    if (!deletingAnuncioId) return;
    try {
      await api.deleteAnuncio(String(deletingAnuncioId));
      setAnunciosData((prev) => prev.filter((a) => a.id !== deletingAnuncioId));
    } catch (err) {
      console.error("Error eliminando anuncio:", err.message);
      onToast?.(err.message || "Error al eliminar el anuncio.", "error");
    } finally {
      setDeletingAnuncioId(null);
    }
  };
  ...
  return (
    <>
      ...
      <section className="module-filters-row anuncios-filters-row">
        ...
        <div className="management-condo-field anuncios-date-field">
          <label>Periodo</label>
          <div className="condo-dropdown" onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget)) setAnnouncementDateDropdownOpen(false); }} tabIndex={-1}>
            <button
              type="button"
              className="condo-dropdown-trigger"
              onClick={() => setAnnouncementDateDropdownOpen((open) => !open)}
              aria-expanded={announcementDateDropdownOpen}
            >
              <span className="condo-dropdown-value">
                {dateFilterOptions.find((opt) => opt.value === announcementDateFilter)?.label || "Todos"}
              </span>
              <svg className={`condo-dropdown-chevron${announcementDateDropdownOpen ? " open" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            {announcementDateDropdownOpen && (
              <ul className="condo-dropdown-list" role="listbox">
                {dateFilterOptions.map((opt) => (
                  <li
                    key={opt.value}
                    role="option"
                    aria-selected={announcementDateFilter === opt.value}
                    className={`condo-dropdown-item${announcementDateFilter === opt.value ? " selected" : ""}`}
                    onMouseDown={() => { setAnnouncementDateFilter(opt.value); setAnnouncementDateDropdownOpen(false); }}
                  >
                    {opt.label}
                  </li>
                ))}
          ...
          <article key={anuncio.id} className="anuncio-card">
            ...
            {canManageAnnouncements && (
              <div className="anuncio-actions">
                ...
                <button type="button" className="anuncio-action-btn anuncio-action-delete" title="Eliminar" onClick={() => setDeletingAnuncioId(anuncio.id)}>
      ...
      {deletingAnuncioId && (
        <div className="modal-overlay modal-overlay-centered" onClick={() => setDeletingAnuncioId(null)}>
          <div className="confirm-modal" onClick={(e) => e.stopPropagation()}>
            <div className="confirm-modal-icon confirm-modal-icon-danger" aria-hidden="true">!</div>
            <h2>¿Eliminar este anuncio?</h2>
            <p>Esta acción no se puede deshacer.</p>
            <div className="confirm-modal-actions">
              <button type="button" className="confirm-modal-cancel" onClick={() => setDeletingAnuncioId(null)}>Cancelar</button>
              <button type="button" className="confirm-modal-accept confirm-modal-accept-danger" onClick={confirmDeleteAnuncio}>Eliminar</button>
            </div>
          </div>
        </div>
      )}
